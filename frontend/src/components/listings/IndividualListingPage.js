...
const IndividualListingPage = ({ listingId, onBack }) => {
  ...
  const loadAreaInfo = async () => {
    try {
      ...
      setAreaCities(response.data.cities || []);
    } catch (err) {
  ...
  const renderOverview = () => {
    ...
    return (
      <div className="space-y-6">
        ...
        {/* Zillow Property Market Data */}
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          <div className="p-6 border-b border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-bold text-gray-900">Property Market Data</h2>
                <p className="text-gray-600 mt-1">Live data from Zillow</p>
              </div>
              {loadingZillow ? (
                <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-blue-600"></div>
              ) : zillowData && (
                <a 
                  href={zillowData.zillow_url}
                  target="_blank"
          ...
          {loadingZillow ? (
            <div className="p-8 text-center">
              <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto mb-4"></div>
              <p className="text-gray-600">Loading Zillow data...</p>
            </div>
          ) : zillowData ? (
            <div className="p-6">
              {/* Price Banner */}
              {zillowData.price && (
                <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg p-6 mb-6 text-white">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-sm opacity-90 mb-1">Zillow Estimate</div>
                      <div className="text-4xl font-bold">${zillowData.price.toLocaleString()}</div>
                      {zillowData.price_per_sqft && (
                        <div className="text-sm opacity-90 mt-2">{zillowData.price_per_sqft} per sqft</div>
                      )}
                    </div>
                    {zillowData.price_change && (
                      <div className={`text-right ${zillowData.price_change > 0 ? 'text-green-300' : 'text-red-300'}`}>
                        <div className="text-2xl font-bold">
                          {zillowData.price_change > 0 ? '+' : ''}{zillowData.price_change.toLocaleString()}
                        </div>
              ...
              <div className="grid md:grid-cols-3 lg:grid-cols-4 gap-4 mb-6">
                {zillowData.bedrooms && (
                  <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                    <div className="text-xs text-gray-500 mb-1">🛏️ Bedrooms</div>
                    <div className="text-2xl font-bold text-gray-900">{zillowData.bedrooms}</div>
                  </div>
                )}
                ...
                {zillowData.bathrooms && (
                  <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                    ...
                    <div className="text-2xl font-bold text-gray-900">{zillowData.bathrooms}</div>
                  </div>
                )}
                
                {zillowData.square_feet && (
                  <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                    <div className="text-xs text-gray-500 mb-1">📐 Square Feet</div>
                    <div className="text-2xl font-bold text-gray-900">{zillowData.square_feet}</div>
                  </div>
                )}
                
                {zillowData.year_built && (
                  <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                    <div className="text-xs text-gray-500 mb-1">🏗️ Year Built</div>
                    <div className="text-2xl font-bold text-gray-900">{zillowData.year_built}</div>
                  </div>
                )}
                
                {zillowData.property_type && (
                  <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                    <div className="text-xs text-gray-500 mb-1">🏠 Property Type</div>
                    <div className="text-lg font-bold text-gray-900">{zillowData.property_type}</div>
                  </div>
                )}
                
                {zillowData.architectural_style && (
                  <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                    <div className="text-xs text-gray-500 mb-1">🏛️ Style</div>
                    <div className="text-lg font-bold text-gray-900">{zillowData.architectural_style}</div>
                  </div>
                )}
                
                {zillowData.days_on_market && (
                  <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                    <div className="text-xs text-gray-500 mb-1">📅 Days on Market</div>
                    <div className="text-lg font-bold text-gray-900">{zillowData.days_on_market}</div>
                  </div>
                )}
                
                {zillowData.parking_spaces && (
                  <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                    <div className="text-xs text-gray-500 mb-1">🚗 Parking</div>
                    <div className="text-lg font-bold text-gray-900">{zillowData.parking_spaces}</div>
                  </div>
                )}
              </div>
            ...
            <div className="p-8 text-center">
              ...
              <p className="text-gray-600">Property not found on Zillow</p>
              <p className="text-sm text-gray-500 mt-1">Data may not be available for this address</p>
            </div>
          )}
        </div>
        ...
        {/* Area Demographics & Nearby Cities */}
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          <div className="p-6 border-b border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-bold text-gray-900">Nearby Cities & Demographics</h2>
                <p className="text-gray-600 mt-1">Local area information within 50 km</p>
              </div>
              {loadingAreaInfo && (
                <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-blue-600"></div>
              )}
          ...
          {loadingAreaInfo ? (
            ...
            <div className="p-6">
              <div className="space-y-4">
                {areaCities.map((city, index) => (
                  <div key={index} className={`border rounded-lg p-4 ${index === 0 ? 'border-blue-300 bg-blue-50' : 'border-gray-200'}`}>
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      {city.population && (
                        <div className="bg-white rounded p-3 border border-gray-200">
                          ...
                          <div className="text-lg font-semibold text-gray-900">{city.population.toLocaleString()}</div>
                        </div>
                      )}
                      
                      <div className="bg-white rounded p-3 border border-gray-200">
                        <div className="text-xs text-gray-500 mb-1">📍 Distance</div>
                        <div className="text-lg font-semibold text-gray-900">{city.distance.toFixed(1)} km</div>
                      </div>
                      
                      {city.elevation !== null && city.elevation !== undefined && (
                        <div className="bg-white rounded p-3 border border-gray-200">
                          <div className="text-xs text-gray-500 mb-1">⛰️ Elevation</div>
                          <div className="text-lg font-semibold text-gray-900">{city.elevation}m</div>
                        </div>
                      )}
                      ...
                      {city.timezone && (
                        <div className="bg-white rounded p-3 border border-gray-200">
                          ...
                          <div className="text-sm font-semibold text-gray-900">{city.timezone}</div>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              {areaCities.length > 0 && (
                <div className="mt-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
                  <p className="text-sm text-gray-600 text-center">
                    📊 Showing {areaCities.length} nearby {areaCities.length === 1 ? 'city' : 'cities'} sorted by population
                  </p>
                </div>
              )}
            </div>
